	...
	if ( ! class_exists( 'Plugin' ) ) {
		...
		class Plugin {
			...
			public function load_textdomain(): bool {
				$this->debug( 'Attempting to load text domain from ' . Helpers::plugins_path( '/lang' ) );

				return load_plugin_textdomain( 'post-content-shortcodes', false, Helpers::plugins_path( '/lang' ) );
			}
			...
			public function print_styles() {
				...
				if ( $this->settings['use-styles'] ) {
					wp_enqueue_style( 'pcs-styles', Helpers::plugins_url( '/lib/styles/ten321/post-content-shortcodes/default-styles.css' ), array(), self::$version, 'screen' );
				}
			}
			 ...
			 * @since  0.1
			 */
			protected function is_multinetwork(): bool {
			 ...
			 * @since  0.1
			 */
			private function _get_attributes( $atts = array() ): array {
			 ...
			 * @since  0.1
			 */
			public function get_id_from_post_name( string $post_name, int $blog = 0 ): bool|int {
				global $blog_id, $wpdb;
			 ...
			 * @since  0.1
			 */
			public function post_content( array $atts = array() ): string {
			 ...
			 *
			 * @param $p \WP_Post|null the Post object
			 *
			 * @access public
			 * @return string the rendered View content template
			 * @since  0.6
			 */
			public function do_view_template( ?\WP_Post $p = null ): string {
				if ( ! function_exists( 'render_view_template' ) ) {
					return '';
				}
				if ( empty( $p ) ) {
			 ...
			 * @since  0.1
			 */
			public function post_list( array $atts = array() ): string {
			 ...
			 * @since  0.1
			 */
			public function get_post_from_blog( int $post_id = 0, int $blog_id = 0 ): ?\WP_Post {
			 ...
			 * @since  0.1
			 */
			public function get_posts_from_blog( array $atts = array(), int $blog_id = 0 ): array {
				if ( empty( $this->shortcode_atts['image_height'] ) && empty( $this->shortcode_atts['image_width'] ) ) {
					$image_size = apply_filters( 'post-content-shortcodes-default-image-size', 'thumbnail', $this->shortcode_atts );
				} else {
					if ( empty( $this->shortcode_atts['image_height'] ) ) {
			 ...
			 *
			 * @param array|null $tax_query the tax_query argument for the query
			 * @param string|null $post_type the type of post/object being queried
			 *
			 * @access public
			 * @return void
			 * @since  0.3.4.1
			 */
			public function check_taxonomies( ?array $tax_query = null, ?string $post_type = null ) {
				if ( empty( $tax_query ) ) {
					return;
				}
				if ( empty( $post_type ) ) {
			 ...
			 * @param array|string $image_size the size of the image to retrieve
			 ...
			 * @since  0.1
			 */
			function get_the_post_thumbnail( int $post_ID, array|string $image_size = 'thumbnail', array $attr = array(), int $blog_id = 0 ): string {
			 ...
			 * @access public
			 */
			public function do_comments( \WP_Post $newpost ): string {
				global $post;
				if ( is_object( $post ) ) {
					$tmpp = clone $post;
				}
			 ...
			 * @uses $wpdb
			 */
			public function get_shortlink_from_blog( int $post_id = 0, int $blog_id = 0 ): string {
				if ( empty( $post_id ) ) {
					return '';
				}
			 ...
			 * @since  0.1
			 */
			public function get_args( array $atts = array() ): array {
				if ( ! is_array( $atts ) ) {
			 ...
			 *
			 * @param string|null $html the current HTML for the post
			 *
			 * @access public
			 * @return null|string
			 * @since  0.6
			 */
			public function do_post_thumbnail( string $html = null ): ?string {
			 ...
			 * @since  0.5
			 */
			public function do_post_permalink( string $link ): string {
			 ...
			 * @since  0.1
			 */
			public function do_entry_classes( array $atts = array() ): string {
			 ...
			 * @since  1.0
			 */
			protected function debug( string $message, bool $log = true ) {
				Helpers::log( $message );
			}
		}
	}
